        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1400 }}>
            <thead>
              {/* 第一级表头：类别分组 */}
              <tr>
                <th rowSpan={2} style={{ ...th, width: 38, position: 'sticky', left: 0, background: '#F9FAFB', zIndex: 3, boxSizing: 'border-box' }}>#</th>
                <th rowSpan={2} style={{ ...th, textAlign: 'left', minWidth: 110, position: 'sticky', left: 38, background: '#F9FAFB', zIndex: 3, boxShadow: '-2px 0 4px rgba(0,0,0,0.06)' }}>村社</th>
            ...
            <tbody>
              {pagedVillages.map((row, i) => (
                <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#FAFAFA' }}>
                  <td style={{ ...td({ textAlign: 'center', color: '#9CA3AF', fontSize: 11, fontWeight: 500 }), position: 'sticky', left: 0, background: i % 2 === 0 ? 'white' : '#FAFAFA', zIndex: 2, borderRight: '1px solid #E5E7EB', boxSizing: 'border-box' }}>{(page - 1) * pageSize + i + 1}</td>
                  <td style={{ ...td({ fontWeight: 600, color: '#111827', whiteSpace: 'nowrap' }), position: 'sticky', left: 38, background: i % 2 === 0 ? 'white' : '#FAFAFA', zIndex: 2, boxShadow: '-2px 0 4px rgba(0,0,0,0.06)', boxSizing: 'border-box' }}>{row.village}</td>
                  {/* 日常检查 */}
