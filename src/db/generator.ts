...
const HAZARD_LEVELS: HazardLevel[] = ['major', 'high', 'general']
...
// 企业字号
const NAME_CHARS_1 = ['鑫', '华', '永', '宏', '盛', '金', '祥', '福', '顺', '天', '安', '诚', '信', '达', '通', '广', '正', '泰', '瑞', '昌', '润', '丰', '裕', '腾', '飞', '龙', '凤', '长', '大', '中', '新', '兴', '凯', '博', '远', '恒', '伟', '强', '光', '明', '佳', '美', '德', '建', '国', '民', '力', '优', '特']
const NAME_CHARS_2 = ['盛', '达', '昌', '隆', '兴', '源', '茂', '业', '邦', '泰', '和', '祥', '瑞', '丰', '伟', '强', '刚', '勇', '毅', '俊', '峰', '涛', '杰', '军', '辉', '明', '鑫', '诚', '信', '德', '仁', '义', '礼', '智', '勇', '文', '武', '斌', '才', '成', '栋', '梁', '材', '林', '森', '松', '柏', '海', '江', '河', '湖', '川', '山']
// 行业词
const INDUSTRY_WORDS = ['化工', '新材料', '建材', '金属制品', '物流', '制药', '机械', '电子', '纺织', '食品', '服装', '包装', '印刷', '塑胶', '家具', '陶瓷', '钢结构', '电缆', '阀门', '轴承', '模具', '铸造', '仓储', '贸易', '科技', '实业', '投资', '建设', '工程', '环保', '运输', '租赁', '物业', '酒店', '餐饮', '汽配', '五金', '橡塑', '纸制品', '印染']
// 组织形式
...
function generateEnterpriseName(): string {
  const hasRegion = Math.random() < 0.4
  const region = hasRegion ? REGIONS[randomInt(0, REGIONS.length - 1)] : ''
  const nameLen = randomInt(2, 4)
  let brandName = ''
  for (let i = 0; i < nameLen; i++) {
    const pool = i % 2 === 0 ? NAME_CHARS_1 : NAME_CHARS_2
    brandName += pool[randomInt(0, pool.length - 1)]
  }
  const industry = INDUSTRY_WORDS[randomInt(0, INDUSTRY_WORDS.length - 1)]
...
export function generateAllData(): GeneratedData {
  ...
  enterprises.forEach(ent => {
    ...
    for (let i = 0; i < hazardCount; i++) {
      ...
      hazards.push({
        ...
        description: `隐患详情描述：${HAZARD_TITLES[randomInt(0, HAZARD_TITLES.length - 1)]}，需要及时整改`,
  ...
  const expertPlatformBehaviors: ExpertPlatformBehavior[] = experts.map(exp => {
    ...
    return {
      ...
      hazard_serious: expHazards.filter(h => h.level === 'major').length,
